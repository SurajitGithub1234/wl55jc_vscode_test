...
void fnRTOSMessageSendTask(void const *argument)
{
  uint32_t u32SendThreadFlag;
  while (1)
  {
    HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_15);
    printf("Test CMSIS-RTOS 1: Sender Task\r\n");
    u32SendThreadFlag = osSignalClear(ReceiverTaskID, 0x00000011);
    u32SendThreadFlag = osSignalSet(ReceiverTaskID, 0x00000011);
    printf("Previous signal flag for the Receiver Thread: %ld\r\n", u32SendThreadFlag);
    osDelay(2000);
  }
}
...
{
  osEvent event;
  uint32_t u32WaitForSignalFlag = 500;
  uint32_t u32SendThreadFlag;
  ;
  while (1)
  {
    ...
    event = osSignalWait(0x00000003, osWaitForever);
    if (event.status == osEventSignal)
    {
      printf("Signal state after reception: %ld\r\n", event.value.signals);
      if (event.value.signals != 0x00)
      {
        printf("Signal flag not cleared upon receiced!!\r\n");
        // u32SendThreadFlag = osSignalClear(ReceiverTaskID, 0x00000003);
        printf("Signal state after reception: %ld\r\n", event.value.signals);
        // Error_Handler();
      }
      else
      {
        printf("Signal flag cleared upon reception\r\n");
      }
    }
    else
    {
      printf("Thread signal not received\r\n");
    }
  }
}
/**
